...
class PredefinedWorkoutsRunScreen extends StatefulWidget {
  ...
  const PredefinedWorkoutsRunScreen({super.key, required this.workout});

  @override
  State<PredefinedWorkoutsRunScreen> createState() =>
      _PredefinedWorkoutsRunScreenState();
}

class _PredefinedWorkoutsRunScreenState
    extends State<PredefinedWorkoutsRunScreen> {
  ...
  CustomWorkoutExercise get _currentExercise =>
  ...
  void _showRepsSelector() {
    ...
    showDialog<int>(
      ...
      builder: (BuildContext context) {
        return RepsSelector(initialReps: _currentExercise.suggestedReps);
      },
    ).then((reps) {
      if (!mounted) return;
      if (reps == null) {
    ...
    if (_currentSetIndex == 0) {
      ...
      context.read<WorkoutBloc>().add(
        ExerciseAdded(
          ...
          muscleGroup:
              exerciseMeta?.muscleGroup ??
              (widget.workout.muscleGroups.isNotEmpty
  ...
  void _moveToNextSet() {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder:
            (context) => PredefinedWorkoutBreakScreen(
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder:
            (context) => PredefinedWorkoutBreakScreen(
              restTime:
                  widget.workout.exercises[_currentExerciseIndex - 1].restTime,
